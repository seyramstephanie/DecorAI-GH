...
import { API, authHeaders } from "../constants/api";
...
export default function DecoratorsScreen() {
  ...
  const fetchDecorators = async () => {
    try {
      const response = await fetch(API.decorators, {
        headers: authHeaders(),
      });
      const data = await response.json();
      setDecorators(data);
    } catch (e) {
      console.error("Error fetching decorators:", e);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading decorators...</Text>
        </View>
      ) : (
        <ScrollView
          ...
          <View style={styles.cardsList}>
            {filtered.length === 0 ? (
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyText}>No decorators found</Text>
              </View>
            ) : (
              filtered.map((dec) => (
                <View key={dec.id} style={styles.card}>
                  ...
                  <View style={styles.cardTop}>
                    ...
                    <View style={styles.cardInfo}>
                      ...
                      <Text style={styles.area}>📍 {dec.location}</Text>
                      {dec.priceMin && dec.priceMax && (
                        <Text style={styles.price}>
                          GHS {dec.priceMin} - {dec.priceMax}
                        </Text>
                      )}
                    </View>
                  </View>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      router.push({
                        ...
                        params: { decoratorId: dec.id, name: dec.name },
                      })
                    }
                  >
...
const styles = StyleSheet.create({
  ...
  decName: { fontSize: 16, fontWeight: "800", color: Colors.text },
  specialtyBadge: {
    borderRadius: 8,
  ...
  specialtyText: { fontSize: 11, fontWeight: "700", color: Colors.primary },
  ratingRow: { flexDirection: "row", alignItems: "center", gap: 4 },
  star: { fontSize: 13 },
  ratingNum: { fontSize: 14, fontWeight: "700", color: Colors.text },
  area: { fontSize: 13, color: Colors.textMuted },
  ...
  profileBtnText: { fontSize: 14, fontWeight: "700", color: Colors.white },
  tabBar: {
    backgroundColor: Colors.white,
    borderTopWidth: 1,
  ...
  tabLabel: { fontSize: 11, fontWeight: "500", color: Colors.textLight },
